...
def analyze_pair(base_name, final_path, keyword_path):
    ...
    # Session-level sentiment (based on participant speech)
    session_sentiments = {}
    for sid in df["Session"].unique():
        p_text = " ".join(df_p[df_p["Session"] == sid]["Content"].astype(str))
        session_sentiments[sid] = detect_sentiment(p_text)
    ...
    for _, kw_row in expanded.iterrows():
        ...
        for sid in df["Session"].unique():
            subset_i = df_i[df_i["Session"] == sid]
            subset_p = df_p[df_p["Session"] == sid]

            participant = next((s for s in subset_p["Speaker"].unique() if s.startswith("P")), None)
            if not participant:
                continue
            ...
            # Sentiment
            sentiment_session = session_sentiments.get(sid, "neutral")
